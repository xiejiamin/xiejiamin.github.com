<?php 
	...
	//获取数据
	$phone=$_GET["usermobile"];
	$pwd=$_GET["password"];
	$repwd=$_GET["confirmPassword"];
    
	//检测
	if($phone=='')
		page_redirect("请输入手机号！",'../forget.html',1);
	...
	if(mysqli_stmt_fetch($stmt))
	{
		...
		$sql="update users set user_pwd=? where user_phone=?";
		$stmt=mysqli_prepare($conn,$sql);
		mysqli_stmt_bind_param($stmt,"ss",$pwd,$phone);
		mysqli_stmt_execute($stmt);
	...
	{
		page_redirect("手机号码不存在！",'../forget.html',1);
	}
	mysqli_stmt_close($stmt);
	mysqli_close($conn);
